...
export function assertFixedPrice(price: RidePrice): asserts price is FixedPrice {
  if (!isFixedPrice(price)) {
    throw new Error(`Price is not a fixed price: ${price}`);
  }
}

export function assertRangePrice(price: RidePrice): asserts price is RangePrice {
